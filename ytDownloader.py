...
def install_ffmpeg():
    """ Provide instructions to install FFmpeg based on the OS. """
    if sys.platform == "win32":
        print("Download FFmpeg from https://ffmpeg.org/download.html and add it to your system path.")
    elif sys.platform == "linux":
        print("Run 'sudo apt-get install ffmpeg' or use a similar command based on your Linux distribution.")
    elif sys.platform == "darwin":
        print("Run 'brew install ffmpeg' if you have Homebrew installed, otherwise download from https://ffmpeg.org/download.html")

def check_ffmpeg():
    """ Check and react to FFmpeg installation status. """
...
def check_pip():
    """ Check and suggest actions for pip installation. """
    if not check_command(['pip', '--version']):
        print("pip is not installed.")
        print("Download and install Python from https://python.org which includes pip.")
    else:
...
def get_ffmpeg_path():
    ffmpeg_path = 'ffmpeg'
    if sys.platform == 'win32':
        ffmpeg_path = 'C:\\ProgramData\\chocolatey\\lib\\ffmpeg-full\\tools\\ffmpeg\\bin\\ffmpeg.exe'
...
def main():
    try:
        print("Checking for required installations...\n")
